        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('rapports.store') }}" method="POST">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <select name="mois" class="form-select @error('mois') is-invalid @enderror">
                                @foreach([1=>'Janvier',2=>'Février',3=>'Mars',4=>'Avril',5=>'Mai',6=>'Juin',
                                         ...
                                         as $n => $nom)
                                    <option value="{{ $n }}"
                                        @selected(old('mois', request('mois', now()->month)) == $n)>
                                        {{ $nom }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="annee" class="form-select @error('annee') is-invalid @enderror">
                                @foreach($annees as $a)
                                    <option value="{{ $a }}"
                                        @selected(old('annee', request('annee', now()->year)) == $a)>
                                        {{ $a }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <select name="province_id" class="form-select @error('province_id') is-invalid @enderror">
                            ...
                            @foreach($provinces->groupBy(fn($p) => $p->region->nom) as $regionNom => $provs)
                                <optgroup label="{{ $regionNom }}">
                                    @foreach($provs as $p)
                                        <option value="{{ $p->id }}"
                                            @selected(old('province_id', request('province_id')) == $p->id)>
                                            {{ $p->nom }}
                                        </option>
                                    @endforeach
                                </optgroup>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Montant sur Caisse (FCFA) <span class="text-danger">*</span></label>
